    ...
    For Index = 0 To 39
        If Iterations > 1 Then
            OutputString = OutputString & " " _
                & FixedLength(CStr(Index + 1), 2) & "  " _
                & FixedLength(Format(cStat(0, Index), "0.00"), 6) & " " _
                & FixedLength(Format(cStat(1, Index), "0.00"), 6) & "  " _
                & FixedLength(Format(cStat(2, Index), "0.00"), 6) & "  " _
                & FixedLength(Format(cStat(3, Index), "0.00"), 6) & "  " _
                & FixedLength(Format(cStat(4, Index), "0.00"), 6) & vbNewLine
        Else
            OutputString = OutputString & " " _
                & FixedLength(CStr(Index + 1), 2) & "  " _
                & FixedLength(cStat(0, Index), 3) & "  " _
                & FixedLength(cStat(1, Index), 3) & "  " _
                & FixedLength(cStat(2, Index), 3) & "  " _
                & FixedLength(cStat(3, Index), 3) & "  " _
                & FixedLength(cStat(4, Index), 3) & vbNewLine
        End If
        
        If (Index + 1) / 3 = Fix((Index + 1) / 3) Then
...
Private Sub cmdEstimate_click()
    ...
    If txtLevel.Text = "0" Then
        MsgBox "Level 0 is not a valid level", vbOKOnly
        Exit Sub
    End If
    
    If CharacterStats.cboName.ListIndex < 0 Then
        CharacterStats.cboName.ListIndex = 0
    End If
    ...
    If Iterations > 1 Then
        OutputString = " LV      HP     MP     ATK     DEF     AGI" & vbNewLine & vbNewLine
        OutputString = OutputString & " " _
            & FixedLength(txtLevel.Text, 2) & "  " _
            & FixedLength(Format(iStat(0), "0.00"), 6) & " " _
            & FixedLength(Format(iStat(1), "0.00"), 6) & "  " _
            & FixedLength(Format(iStat(2), "0.00"), 6) & "  " _
            & FixedLength(Format(iStat(3), "0.00"), 6) & "  " _
            & FixedLength(Format(iStat(4), "0.00"), 6)
    Else
        OutputString = " LV   HP   MP  ATK  DEF  AGI" & vbNewLine & vbNewLine
        OutputString = OutputString & " " _
            & FixedLength(txtLevel.Text, 2) & "  " _
            & FixedLength(iStat(0), 3) & "  " _
            & FixedLength(iStat(1), 3) & "  " _
            & FixedLength(iStat(2), 3) & "  " _
            & FixedLength(iStat(3), 3) & "  " _
...
Private Sub cmdRange_Click()
    ...
    For Index = 0 To 39
        OutputString = OutputString & " " _
            & FixedLength(CStr(Index + 1), 2) & "  " _
            & FixedLength(RangeLow(0, Index) & "-", 4) _
            & LeftLength(RangeHigh(0, Index), 3) & "  " _
            & FixedLength(RangeLow(1, Index) & "-", 4) _
            & LeftLength(RangeHigh(1, Index), 3) & "  " _
            & FixedLength(RangeLow(2, Index) & "-", 4) _
            & LeftLength(RangeHigh(2, Index), 3) & "  " _
            & FixedLength(RangeLow(3, Index) & "-", 4) _
            & LeftLength(RangeHigh(3, Index), 3) & "  " _
            & FixedLength(RangeLow(4, Index) & "-", 4) _
            & LeftLength(RangeHigh(4, Index), 3) & vbNewLine
...
Private Sub Initialize_Growths()
    ...
    Values = "313 352 352 352 352 313 352 352 352 352 352 313 352 352 352 352 352 313 352 352 352 352 352 313 352 352 352 352 352"
    'Values = "3.13 3.52 3.52 3.52 3.52 3.13 3.52 3.52 3.52 3.52 3.52 3.13 3.52 3.52 3.52 3.52 3.52 3.13 3.52 3.52 3.52 3.52 3.52 3.13 3.52 3.52 3.52 3.52 3.52"
    
    strArray = Split(Values, " ")
    
    For Index = 0 To 28
        Gro_Curve(1, Index) = CCur(strArray(Index) / 100)
    Next Index
    
    Values = "234 273 273 273 313 273 313 313 313 313 313 313 352 352 352 352 352 352 352 391 391 391 391 352 430 391 430 391 469"
    'Values = "2.34 2.73 2.73 2.73 3.13 2.73 3.13 3.13 3.13 3.13 3.13 3.13 3.52 3.52 3.52 3.52 3.52 3.52 3.52 3.91 3.91 3.91 3.91 3.52 4.30 3.91 4.30 3.91 4.69"
    ...
    Values = "430 430 391 430 391 352 430 352 391 391 391 313 352 352 352 352 352 313 313 313 313 352 273 273 313 273 273 313 234"
    strArray = Split(Values, " ")
    ...
    Values = "234 273 313 313 313 273 352 352 391 352 391 352 430 430 430 430 430 352 391 352 391 352 352 273 313 313 313 273 273"
    strArray = Split(Values, " ")
    ...
    Values = "469 469 430 430 391 313 352 313 313 313 273 273 234 273 273 273 234 273 273 313 313 313 352 313 391 430 430 469 508"
    strArray = Split(Values, " ")
            ...
            If Index < 31 Then
                For Sindex = 0 To 4
                    If CurveType(Sindex) > 0 Then
                        Stat(Sindex) = Fix(Stat(Sindex) + GrowStat(Sindex) _
                            * (Gro_Curve(CurveType(Sindex), Index - 2) / 100) _
                            + RanNum(1, 99999) / 100000)
